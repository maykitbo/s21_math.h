...
START_TEST(sin_test_1) {
    for (int R = 20; R >= -20; R -= 4) {
        for (double k = 2 * s21_M_PI; k > -2 * s21_M_PI; k -= s21_M_PI / 17) {
            ck_assert_double_eq_tol(sin(k + R * s21_M_PI), s21_sin(k + R * s21_M_PI), 1e-6);
        }
    }
}
END_TEST
...
START_TEST(cos_test_1) {
    for (int R = 20; R >= -20; R -= 4) {
        for (double k = 2 * s21_M_PI; k > -2 * s21_M_PI; k -= s21_M_PI / 17) {
            ck_assert_double_eq_tol(cos(k + R * s21_M_PI), s21_cos(k + R * s21_M_PI), 1e-6);
        }
    }
}
END_TEST
...
START_TEST(tan_test_1) {
    for (int R = 20; R >= -20; R -= 4) {
        for (double k = 2 * s21_M_PI; k > -2 * s21_M_PI; k -= s21_M_PI / 9) {
            ck_assert_double_eq_tol(tan(k + R * s21_M_PI), s21_tan(k + R * s21_M_PI), 1e-6);
        }
    }
}
END_TEST
